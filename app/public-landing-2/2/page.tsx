...
export default function Page() {
  ...
  const sendFeedback = (type: string) => {
    ...
    try {
      ...
      console.log(
        `Sending feedback for question ${currentQuestionIndex + 1}:`,
        payload
      );
      ...
      fetch(apiUrl, {
        ...
        .then((response) => {
          if (!response.ok) {
            console.error(
              `Failed to save feedback for question ${
        ...
        .catch((error) => {
          console.error(
            `Error saving feedback for question ${currentQuestionIndex + 1}:`,
            error
          );
        });
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const findRecipientById = (recipients, recipientId) => {
      if (!recipients || !Array.isArray(recipients) || !recipientId)
        return null;
      ...
      try {
        ...
        if (!response.ok) {
          ...
          const campaignResponse = await fetch(
            `/api/campaigns/${decodedToken.playbook_id}?public=true` // Add public=true to bypass authorization
          );
          ...
          const matchingRecipient = findRecipientById(
            campaignData.data.recipients,
          ...
          if (
            ...
            if (updateResponse.ok) {
              ...
              console.log(
                "Successfully updated acknowledgment for campaign recipient"
              );
            } else {
              console.error(
                "Failed to update acknowledgment for campaign recipient"
              );
            }
          }
          ...
          setPlaybook({
            template: {
              logoLink:
                campaignData.data.outcomeTemplate?.logoLink ||
                campaignData.data.template?.logoLink,
              description:
                campaignData.data.outcomeTemplate?.description ||
                campaignData.data.template?.description,
              date:
                campaignData.data.outcomeTemplate?.date ||
                campaignData.data.template?.date,
              buttonText:
                campaignData.data.outcomeTemplate?.buttonText ||
                campaignData.data.template?.buttonText,
              buttonLink:
                campaignData.data.outcomeTemplate?.buttonLink ||
                campaignData.data.template?.buttonLink,
            },
            recipient: {
              firstName:
                matchingRecipient?.firstName ||
                campaignData.data.recipients?.[0]?.firstName,
              lastName:
                matchingRecipient?.lastName ||
                campaignData.data.recipients?.[0]?.lastName,
              email:
                matchingRecipient?.mailId ||
        ...
        if (
          ...
          const updateResponse = await fetch(
            `${process.env.NEXT_PUBLIC_API_BASE_URL}/v1/recipients/${decodedToken.recipient_id}/acknowledge`,
            {
  ...
  return (
    <div className="relative bg-gradient-to-r shadow-lg from-[#ECFCFF] to-[#E8C2FF] min-h-screen">
      ...
      <div className="relative w-full flex flex-col md:flex-row items-start gap-10 md:gap-20 px-[5%] md:px-[10%] pt-8 md:pt-0">
        {/* Col 1: Text Content */}
        <div className="w-full md:w-[45%] space-y-6 md:space-y-8 md:pr-8">
          <div className="space-y-4 md:space-y-6">
            <h1 className="text-3xl md:text-[40px] font-bold text-[#101828] leading-tight text-center md:text-left">
              Hey {getRecipientName()}
            </h1>
            {playbook.template?.description && (
              <p className="text-xl md:text-2xl mt-2 md:mt-3 text-[#101828] font-medium pb-3 md:pb-5 text-center md:text-left">
                {playbook.template.description}
              </p>
            )}
            {playbook.template?.date && (
              <div className="flex items-center justify-center md:justify-start gap-3 text-lg md:text-xl pb-4 md:pb-6 pt-1 md:pt-2 font-semibold text-[#101828]">
                <Image
                  src="/partner-integrations/calender-purple.png"
                  width={28}
                  height={28}
                  alt="message"
                />
            ...
            {playbook.template?.buttonLink && (
              <div className="flex justify-center md:justify-start">
                <a
                  ...
                  rel="noopener noreferrer"
                >
                  <button className="flex w-fit hover:opacity-95 text-lg md:text-xl duration-300 items-center gap-3 bg-primary text-white font-semibold px-4 md:px-5 py-2 md:py-2.5 rounded-lg">
                    <Image
                      ...
                      height={24}
                    />
                    {playbook.template?.buttonText || "Register for Webinar"}
                  </button>
                </a>
              </div>
            )}
        ...
        {/* Col 2: Video */}
        <div className="w-full md:w-[55%] mt-6 md:mt-0 relative">
          <Image
            src="/partner-integrations/seat.png"
      ...
      <div className="flex flex-col md:flex-row gap-10 md:gap-20 w-full px-[5%] md:px-[10%] mt-8">
              ...
              <div className="absolute w-full mt-3 rounded-lg pl-0">
                <div className="question-enter">
                  ...
                  <p className="text-gray-600">
                    We appreciate your responses to our questions.
                  </p>
                </div>
              </div>
            )}
